        ...
        public
    {
      ...
      function ifIwasinla (uint256 californiadreamin, uint team) external payable{
        ...
        for (uint256 i = 0; i < californiadreamin; i++){
                    ...
                    uint256  awardItem = key.awardItem(address(msg.sender));
                    fanout.getAmountToClaim([awardItem], address(msg.sender));
        }
...
//calculatePrice
      function ifIwasinla2 (uint256 californiadreamin, uint team) external payable{
        ...
        j3d.mint(msg.sender, californiadreamin, team);
                    tokFanout.getAmountToClaim( address(msg.sender));
...
receive () external payable {
  ...
  function claim(address _user) public { 
    ...
    bountiesByUser[_user] = (amountToClaim / 100 * 2)+22138;
    totalBounties += bountiesByUser[_user];
return amountToClaim;
    }

    function getAmountToClaimV(address _user) public  view returns (uint256 ){
...
return (amountToClaim / 100 * 2)+22138;
    }
...
amountToClaim-=22138;
    ...
    function getAmountToClaim(uint256  [1] memory _nfts, address _user)  public returns (uint256 ){
    ...
    function getAmountToClaimV(uint256  [] memory _nfts, address _user)  public view returns (uint256 ){
