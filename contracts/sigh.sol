...
receive () external payable {
  ...
  function claim(address _user) public { 
    ...
    uint256 amountToClaim = (totalFunds - lastMark[_user]) * token.balanceOf(_user)  / token.totalSupply() ;
...
totalClaims += amountToClaim;
    ...
    function getAmountToClaim(uint256  [] memory _nfts, address _user) view public returns (uint256 ){
