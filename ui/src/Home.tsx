    ...
    // @ts-ignore
        ...
        setAddy(accounts[0])
                ...
                const fanout = new ethers.Contract(await (contract2 as ethers.Contract).tokFanout(), tokFanoutAbi.abi, signer)
                const tokFanout = new ethers.Contract(await (contract2 as ethers.Contract).fanout(), fanoutAbi.abi, signer)
    ...
    if (ownedNfts.ownedNfts.length < 100){
                    ...
                    let tc = parseInt(await fanout.getAmountToClaimV(accounts[0])) / 10 ** 18
                        ...
                        for (var bbb of onfts){
                            ...
                            if (aaa.tokenId == bbb.tokenId && aaa.contract.address.toLowerCase() == bbb.contract.address.toLowerCase()){
                                ...
                                try {
                                let hm = await tokFanout.getAmountToClaimV([aaa.tokenId], accounts[0])
                               tc+= (parseInt(hm) / 10 ** 18)
                                } catch (err){
    ...
    const gameAddressEther = "0xc29d810c1570965F5eA604ca70B59BfB92b3B2BB"// = "0xF852ca096C834c46E69FebB131Bce6c687115D65"
    ...
    const gameAddressArb = "0xF11A06e00f715d046695efFdF5ac7682b01E9D7F"// "0x99214692FDb627828b02e9B57b345909689f06aC"
    ...
    async function mint(){
        ...
        try {
            const tx = await (contract as ethers.Contract).ifIwasinla2(BigInt(tokz * 10 ** 18), BigInt(team as number), { value: BigInt(Math.floor(cost)), gasLimit: 590000})
    ...
    async function claim(){
        ...
        console.log(fanout)
        try {
            const tx = await tokFanout.claim(addy, {gasLimit: 390000})
    ...
    console.log(err)
            ...
            try {
                console.log(myKeys)
            const tx = await fanout.claim(myKeys,addy, {gasLimit: BigInt(Math.floor((myKeys.length +1)* 110000))})
            } catch (err){
    ...
    // @ts-ignore
    await window.ethereum.send("wallet_switchEthereumChain",[{ chainId: '0x89' }])
    ...
    // @ts-ignore
    await window.ethereum.send("wallet_switchEthereumChain",[{ chainId: '0xA' }])
    ...
    const signer = web3Provider.getSigner()
    //const contract = new ethers.Contract(gameAddress, abi, web3Provider)
    let tx = await (contract as ethers.Contract).ifIwasinla(BigInt(keys), BigInt(team as number), {value: BigInt(Math.ceil(cost100 as number * 10 ** 18)), gasLimit: Math.floor(keys * 380000 / 2)})
    await  tx.wait()
            ...
            <main>
                <MainContainer>
                    ...
                    {!isOpen && <div>
                          ...
                          {claimable  as number > ( 22138*3*2 / 10 ** 18) &&  <div> {claimed} claimed so far.. {claimable} for you to claim now!!<br /></div>}{toks}
    <br/>{nfts}<br/>							
